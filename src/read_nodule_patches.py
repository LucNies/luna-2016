...
class NoduleReader:
    ...
    """
    def __init__(self, batch_size = 1100, shuffle = True, class_balance = 0.2, meta_data = 'image_stats.stat', label_path = lbl_path, patch_shape = (64,64)):
    ...
    def next(self):
        if self.current >self.n_samples-1:
            ...
            while n_batches < self.batch_size:
                ...
                for i, label in enumerate(labels):
                    ...
                    if label.sum() > 0: # so there is a nodule in the slice
                        image_patches, image_labels = self.patch(batch[i], label, n_patches)
                        patch_batch[n_batches*n_patches:n_batches+n_patches] = image_patches
                        patch_labels[n_batches*n_patches:n_batches+n_patches] = image_labels
                        n_batches += 1
                ...
                if self.current > self.n_samples-1:
                    patch_batch = patch_batch[:n_batches]
                    patch_labels = patch_labels[:n_batches]
    ...
    def patch(self, image, labels, n_patches):
        ...
        patches = self.augment_patches(patches) #Augment all patches (both positive and negative samples)
